 ...
 */
const eitherLeft = Either.Left = S.Left;
const eitherRight = Either.Right = S.Right;
 ...
 */
// :: String -> Maybe a -> a
const explodeMaybe = (errorMessage, maybeObject) => {
  if (maybeObject.isJust) return maybeObject.value;
  throw new Error(errorMessage);
};

// :: [Maybe a] -> [a]
const catMaybies = (maybeObjects) => R.compose(
 ...
 */
// :: Either a b -> b
const explodeEither = foldEither(err => {
  throw new Error(`Explosion failed: ${err}`);
}, R.identity);

// :: Either a b -> Promise a b
const eitherToPromise = foldEither(
  Promise.reject.bind(Promise),
  Promise.resolve.bind(Promise)
);

// :: (() -> b) -> (a -> a) -> Maybe a -> Promise a b
const maybeToPromise = R.curry((nothingFn, justFn, maybe) =>
...
const reduceM = R.curry((m, fn, initial, list) =>
...
const reduceEither = reduceM(Either.of);
...
const reduceMaybe = reduceM(Maybe.of);
...
const leftIf = R.curry((condition, leftFn, val) =>
